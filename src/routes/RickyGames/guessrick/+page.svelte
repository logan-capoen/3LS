<script>
  ...
  function pickRandomCharacter() {
    const index = Math.floor(Math.random() * Ricks.length);
    currentCharacter = Ricks[index];
    guess = '';
    message = '';
  ...
  function nextTry() {
    ...
    // @ts-ignore
    const nameWords = currentCharacter.name.toLowerCase().split(' ').map(normalize);
...
<div class="h-190 flex flex-col items-center justify-center bg-[#1b263b]">
  ...
  {:else if currentCharacter}
    <!-- 👇 Jeu actif -->
    <h1 class="text-[#e0e1dd] text-[40px] mb-15 md:mt-0 mt-20">Qui est ce Rick ?</h1>
    ...
    <div class="relative w-[300px] h-[300px] mb-4">
      ...
      {#if validationStatus === 'correct'}
        ...
        <div class="absolute bottom-4 left-1/2 transform -translate-x-1/2 w-[200px] bg-black/60 rounded-2xl">
          <p class="text-white text-2xl font-bold px-4 py-2 text-center">{currentCharacter.name}</p>
        </div>
